...
use polytorus::blockchain::block::{Block, BuildingBlock, DifficultyAdjustmentConfig, MiningStats};
...
fn main() -> polytorus::Result<()> {
    ...
    let dummy_transaction =
        Transaction::new_coinbase("miner_address".to_string(), "coinbase_data".to_string())?;

    println!("1. Basic mining example:");
    let building_block: BuildingBlock<network::Development> =
        Block::<Building, network::Development>::new_building_with_config(
    ...
    println!(
        "   - Initial difficulty: {}",
        building_block.get_difficulty()
    );
    println!(
        "   - Configured minimum difficulty: {}",
        building_block.get_difficulty_config().min_difficulty
    );
    println!(
        "   - Configured maximum difficulty: {}",
        building_block.get_difficulty_config().max_difficulty
    );
    ...
    let building_block2: BuildingBlock<network::Development> =
        Block::<Building, network::Development>::new_building_with_config(
            vec![dummy_transaction.clone()],
    ...
    let recent_blocks: Vec<&Block<block_states::Finalized, network::Development>> =
        vec![&finalized_block, &finalized_block2];

    let building_block3: BuildingBlock<network::Development> =
        Block::<Building, network::Development>::new_building_with_config(
    ...
    println!(
        "   - Adaptive mining completed! Used difficulty: {}",
        mined_block3.get_difficulty()
    );
