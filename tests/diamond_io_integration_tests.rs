...
async fn test_performance_comparison() {
    ...
    println!(
        "Dummy mode - Obfuscation: {:?}, Evaluation: {:?}",
        dummy_obfuscation_time, dummy_evaluation_time
    );

    // Verify dummy mode is fast (should be under 100ms for basic operations)
    assert_eq!(
        dummy_obfuscation_time.as_millis() < 100,
        true,
        "Dummy mode obfuscation should be fast (< 100ms)"
    );
    assert_eq!(
        dummy_evaluation_time.as_millis() < 100,
        true,
        "Dummy mode evaluation should be fast (< 100ms)"
    );
