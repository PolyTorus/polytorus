...
use crate::{
    blockchain::{
        block::{Block, BuildingBlock, FinalizedBlock},
    },
    config::DataContext,
...
impl PolyTorusConsensusLayer {
    ...
    fn validate_proof_of_work(&self, block: &Block) -> bool {
        // Recreate block hash with current nonce
        ...
        if calculated_hash != block.get_hash() {
            log::warn!("Block hash mismatch: calculated {} != stored {}", calculated_hash, block.get_hash());
            return false;
        }
        
        // Check if hash meets difficulty requirement
        let difficulty_target = "0".repeat(self.config.difficulty);
        let meets_difficulty = calculated_hash.starts_with(&difficulty_target);
        
        if !meets_difficulty {
            log::warn!("Block {} does not meet difficulty requirement: {} zeros", calculated_hash, self.config.difficulty);
        }
    ...
    fn calculate_block_hash(&self, block: &FinalizedBlock) -> String {
        self.calculate_block_hash_with_nonce(block, block.get_nonce())
    }
    
    /// Calculate block hash with a specific nonce
    fn calculate_block_hash_with_nonce(&self, block: &FinalizedBlock, nonce: i32) -> String {
        let mut hasher = Sha256::new();
        
        // Hash block components in deterministic order
        hasher.update(block.get_prev_hash().as_bytes());
        hasher.update(&block.get_timestamp().to_le_bytes());
        hasher.update(&block.get_height().to_le_bytes());
        hasher.update(&nonce.to_le_bytes());
        hasher.update(&block.get_difficulty().to_le_bytes());
        
        // Hash all transactions
        for tx in block.get_transactions() {
            if let Ok(tx_bytes) = serialize(tx) {
    ...
    /// Create a mined block with the given nonce and hash
    fn create_mined_block(&self, original_block: &FinalizedBlock, _nonce: i32, _hash: String) -> FinalizedBlock {
        // Create a new block with the mining results
        // This is a simplified approach - in practice, we'd use the proper block builder
        let new_block = original_block.clone();
        
        // The block struct has private fields, so we need to work around this
        // For now, we'll return the original block (this is a limitation of the current design)
        log::warn!("Block creation is limited due to private fields - using existing block");
        new_block
    }
    ...
    fn validate_transactions(&self, block: &FinalizedBlock) -> bool {
        ...
        if coinbase_count != 1 {
            log::warn!("Block has {} coinbase transactions, expected 1", coinbase_count);
            return false;
        }
    ...
    fn validate_single_transaction(&self, tx: &Transaction, _block: &FinalizedBlock) -> bool {
        ...
        if let Ok(calculated_hash) = tx.hash() {
            if calculated_hash != tx.id {
                log::warn!("Transaction hash mismatch: {} != {}", calculated_hash, tx.id);
                return false;
            }
        } else {
    ...
    fn validate_transaction_inputs_outputs(&self, tx: &Transaction) -> bool {
        ...
        if input_value < output_value {
            log::warn!("Transaction {} has insufficient input value: {} < {}", tx.id, input_value, output_value);
            return false;
        }
    ...
    fn validate_block_size(&self, block: &FinalizedBlock) -> bool {
        if let Ok(block_bytes) = serialize(block) {
            ...
            if block_size > self.config.max_block_size {
                log::warn!("Block size {} exceeds maximum {}", block_size, self.config.max_block_size);
                return false;
            }
        }
        true
    ...
    fn validate_block_timestamp(&self, block: &FinalizedBlock) -> bool {
        ...
        if block_time > max_future_time {
            log::warn!("Block timestamp too far in future: {} > {}", block_time, max_future_time);
            return false;
        }
            ...
            if !prev_hash.is_empty() {
                if let Ok(prev_block) = self.storage.get_block(&prev_hash) {
                    if block_time <= prev_block.get_timestamp() {
                        log::warn!("Block timestamp not greater than previous block: {} <= {}", block_time, prev_block.get_timestamp());
                        return false;
                    }
                }
            }
    ...
    pub fn mine_block(&self, block: &BuildingBlock) -> Result<FinalizedBlock> {
        log::info!("Starting to mine block at height {} with difficulty {}", block.get_height(), self.config.difficulty);
        
        let difficulty_target = "0".repeat(self.config.difficulty);
        let max_nonce = 1_000_000; // Reasonable limit for testing
        
        let start_time = SystemTime::now();
        
        // Convert building block to finalized block for hashing
        let working_block: FinalizedBlock = unsafe { std::mem::transmute(block.clone()) };
        
        for nonce in 0..max_nonce {
            // Create a hash with this nonce by modifying our calculation
            let hash = self.calculate_block_hash_with_nonce(&working_block, nonce);
            
            // Check if hash meets difficulty requirement
            if hash.starts_with(&difficulty_target) {
                // Create a new block with the successful nonce and hash
                let mined_block = self.create_mined_block(&working_block, nonce, hash.clone());
                
                let elapsed = start_time.elapsed().unwrap_or_default();
                log::info!(
                    "Block mined successfully! Hash: {}, Nonce: {}, Time: {:?}",
                    hash, nonce, elapsed
                );
                return Ok(mined_block);
            }
            
            // Log progress every 50000 attempts
            if nonce % 50000 == 0 && nonce > 0 {
                log::debug!("Mining attempt {}: {}", nonce, hash);
            }
        }
        
        Err(anyhow::anyhow!("Mining failed after {} attempts", max_nonce))
    }
...
mod tests {
    ...
    use crate::{
        ...
        crypto::transaction::Transaction,
        test_helpers::{create_test_context, cleanup_test_context},
    };
    ...
    async fn test_real_pow_validation() {
        ...
        let coinbase_tx = Transaction::new_coinbase("test_address".to_string(), "test_data".to_string()).unwrap();
        ...
        // Validate the mined block
        assert!(consensus.validate_block(&mined_block), "Mined block should be valid");
        
        // Check that hash meets difficulty requirement
        let hash = consensus.calculate_block_hash(&mined_block);
    ...
    async fn test_transaction_validation() {
        ...
        let valid_tx = Transaction::new_coinbase("test_address".to_string(), "test_data".to_string()).unwrap();
        
        // Create a block with the transaction and finalize it for validation
        let building_block: BuildingBlock<Mainnet> = BuildingBlock::new_building(
            vec![valid_tx],
            "".to_string(),
            0,
            1,
        ...
        assert!(consensus.validate_transactions(&finalized_block), "Valid transactions should pass validation");
    ...
    async fn test_block_structure_validation() {
        ...
        assert!(consensus.validate_block_structure(&finalized_block), "Valid block structure should pass");
    ...
    async fn test_consensus_layer_creation() {
        ...
        let validator_consensus = PolyTorusConsensusLayer::new(context1.clone(), config.clone(), true).unwrap();
        assert!(validator_consensus.is_validator(), "Node should be configured as validator");
        
        // Test non-validator node creation with separate context
        let non_validator_consensus = PolyTorusConsensusLayer::new(context2.clone(), config, false).unwrap();
        assert!(!non_validator_consensus.is_validator(), "Node should not be configured as validator");
    ...
    async fn test_consensus_builder() {
        ...
        assert!(consensus.is_validator(), "Builder should create validator node");
