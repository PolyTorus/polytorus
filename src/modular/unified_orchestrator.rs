...
impl UnifiedModularOrchestrator {
    ...
    pub async fn process_block(&self, block: Block<block_states::Building, network::Development>) -> Result<Block<block_states::Finalized, network::Development>> {
        ...
        // Update metrics
        {
            ...
            metrics.average_block_time_ms = 
                (metrics.average_block_time_ms * (metrics.total_blocks_processed - 1) as f64 + processing_time) 
    ...
    pub async fn get_detailed_layer_info(&self) -> Result<HashMap<String, String>> {
        ...
        layer_info.insert("execution".to_string(), format!("Execution layer active at {:p}", self.execution_layer.as_ref()));
        
        // Access settlement layer information 
        layer_info.insert("settlement".to_string(), format!("Settlement layer active at {:p}", self.settlement_layer.as_ref()));
        
        // Access consensus layer information
        layer_info.insert("consensus".to_string(), format!("Consensus layer active at {:p}", self.consensus_layer.as_ref()));
        
        // Access data availability layer information
        layer_info.insert("data_availability".to_string(), format!("DA layer active at {:p}", self.data_availability_layer.as_ref()));
    ...
    pub async fn send_message(&self, message_type: String, payload: Vec<u8>) -> Result<()> {
        ...
        println!("📤 Sending message {} (type: {}, size: {} bytes)", 
            message_id, message_type, payload.len());
    ...
    pub async fn broadcast_message(&self, message_type: String, payload: Vec<u8>) -> Result<()> {
        ...
        println!("📡 Broadcasting via message bus at {:p}: {} ({} bytes)", 
            self.message_bus.as_ref(), message_type, payload.len());
    ...
    pub async fn get_performance_stats(&self) -> Result<HashMap<String, f64>> {
        ...
        stats.insert("transactions_per_second".to_string(), transactions_per_second);
        stats.insert("blocks_per_minute".to_string(), blocks_per_minute);
        stats.insert("events_per_second".to_string(), events_per_second);
        stats.insert("pending_transaction_ratio".to_string(), 
        ...
        stats.insert("average_block_time_ms".to_string(), metrics.average_block_time_ms);
    ...
    pub async fn process_priority_events(&self) -> Result<()> {
        ...
        for event in normal_events.into_iter().take(10) { // Limit batch size
    ...
    async fn handle_priority_event(&self, event: UnifiedEvent) -> Result<()> {
        match event {
            UnifiedEvent::SystemAlert { severity, message, component } => {
                eprintln!("🚨 PRIORITY ALERT [{:?}] in {}: {}", severity, component, message);
            ...
            UnifiedEvent::LayerStatusChanged { layer, old_status, new_status } => {
                println!("🔄 Layer {} status: {:?} → {:?}", layer, old_status, new_status);
    ...
    pub async fn run_event_loop(&self) -> Result<()> {
        ...
        while let Some(event) = rx.recv().await {
            if let Err(e) = self.handle_event(event).await {
                ...
                metrics.error_rate = (metrics.error_rate * (total_events - 1) as f64 + 1.0) / total_events as f64;
            }
        }
    ...
    async fn handle_event(&self, event: UnifiedEvent) -> Result<()> {
        match event {
            UnifiedEvent::BlockProposed { block: _, proposer_id, timestamp } => {
                println!("📦 Block proposed by {} at {}", proposer_id, timestamp);
            }
            UnifiedEvent::BlockFinalized { block_hash, block_height, timestamp } => {
                println!("✅ Block finalized: {} (height: {}) at {}", block_hash, block_height, timestamp);
            }
            UnifiedEvent::LayerHealthChanged { layer_type, is_healthy, details } => {
                let status = if is_healthy { "✅" } else { "❌" };
                println!("{} Layer {} health: {}", status, layer_type, details);
            }
            UnifiedEvent::PerformanceAlert { metric, current_value, threshold, severity } => {
                println!("🚨 Performance Alert ({:?}): {} = {} (threshold: {})", 
                    severity, metric, current_value, threshold);
            }
            UnifiedEvent::PerformanceOptimization { optimization_type, metrics_before, metrics_after } => {
                println!("⚙️ Performance Optimization ({}) applied: {} → {}", 
    ...
    pub async fn create_and_start_with_defaults(config: ModularConfig, data_context: crate::config::DataContext) -> Result<Self> {
        ...
        let network_config = super::network::ModularNetworkConfig {
            ...
            bootstrap_peers: config.data_availability.network_config.bootstrap_peers.clone(),
        ...
        let execution_layer = Arc::new(PolyTorusExecutionLayer::new(data_context.clone(), config.execution.clone())?);
        let settlement_layer = Arc::new(PolyTorusSettlementLayer::new(config.settlement.clone())?);
        let consensus_layer = Arc::new(PolyTorusConsensusLayer::new(data_context.clone(), config.consensus.clone(), false)?);
        let data_availability_layer = Arc::new(PolyTorusDataAvailabilityLayer::new(config.data_availability.clone(), network)?);
...
impl UnifiedOrchestratorBuilder {
    ...
    pub fn with_data_availability_layer(mut self, layer: Arc<dyn DataAvailabilityLayer + Send + Sync>) -> Self {
    ...
    pub fn with_config_manager(mut self, config_manager: Arc<RwLock<ModularConfigManager>>) -> Self {
    ...
    pub fn build(self) -> Result<UnifiedModularOrchestrator> {
        let execution_layer = self.execution_layer.ok_or_else(|| failure::err_msg("Execution layer is required"))?;
        let settlement_layer = self.settlement_layer.ok_or_else(|| failure::err_msg("Settlement layer is required"))?;
        let consensus_layer = self.consensus_layer.ok_or_else(|| failure::err_msg("Consensus layer is required"))?;
        let data_availability_layer = self.data_availability_layer.ok_or_else(|| failure::err_msg("Data availability layer is required"))?;
        let message_bus = self.message_bus.ok_or_else(|| failure::err_msg("Message bus is required"))?;
        let config_manager = self.config_manager.ok_or_else(|| failure::err_msg("Config manager is required"))?;
        let layer_factory = self.layer_factory.ok_or_else(|| failure::err_msg("Layer factory is required"))?;
