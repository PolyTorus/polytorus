...
impl StorageLayer for ModularStorage {
    fn store_block(&self, block: &Block) -> Result<Hash> {
        ...
        self.index_db.insert(format!("meta_{}", hash), metadata_data)?;
        
        // Update height index
        let height = block.get_height() as u64;
        self.index_db.insert(format!("height_{}", height), hash.as_bytes())?;
    ...
    fn get_block(&self, hash: &Hash) -> Result<Block> {
        ...
        // Get from database
        let block_data = self.block_db.get(hash)?
    ...
    fn get_block_metadata(&self, hash: &Hash) -> Result<BlockMetadata> {
        ...
        let metadata_data = self.index_db.get(metadata_key)?
    ...
    fn compact(&self) -> Result<()> {
        ...
        log::info!("  Block DB: {} -> {} bytes", block_size_before, block_size_after);
        log::info!("  State DB: {} -> {} bytes", state_size_before, state_size_after);
        log::info!("  Index DB: {} -> {} bytes", index_size_before, index_size_after);
...
mod tests {
    ...
    fn create_test_block(height: i32) -> Block {
        let transactions = vec![Transaction::new_coinbase("test_address".to_string(), "50".to_string()).unwrap()];
        
        Block::new_test_finalized(
            transactions,
            if height == 0 { String::new() } else { format!("prev_hash_{}", height - 1) },
    ...
    fn test_block_exists() {
        ...
        assert!(!storage.block_exists(&"nonexistent_hash".to_string()).unwrap());
    }
