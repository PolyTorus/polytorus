...
mod difficulty_adjustment_tests {
    ...
    fn create_test_block(height: i32, prev_hash: String, difficulty: usize) -> Block<block_states::Building, network::Development> {
    ...
    fn test_block_creation_with_config() {
        ...
        assert_eq!(block.get_difficulty(), 3);
        assert_eq!(block.get_difficulty_config().base_difficulty, 2);
        assert_eq!(block.get_difficulty_config().min_difficulty, 1);
        assert_eq!(block.get_difficulty_config().max_difficulty, 8);
    }
